        ...
        <div className="fixed top-0 left-0 right-0 z-50 bg-[#181611]/90 backdrop-blur-md border-b border-border-dark">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16 sm:h-20">
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        {navLinks.map((link: NavLink) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                className={`text-gray-300 hover:text-primary transition-colors text-sm font-medium ${pathname === link.href ? 'text-primary' : ''}` }
                            >
                                {link.label}
                            </Link>
                        ))}
                    ...
                    {/* Book button (desktop) */}
                    <Link href='/packages' >
                        <button className="hidden md:block bg-primary hover:bg-yellow-300 text-[#181611] text-sm font-bold py-2.5 px-5 rounded-lg transition-colors">
                            Book Safari
                        </button>
                    </Link>
                    {/* Hamburger (mobile) */}
                    ...
                    {open && (
                    <div className="md:hidden absolute top-full left-0 w-full bg-[#12110f] border-t border-primary/20">
                        ...
                        {navLinks.map((link: NavLink) => (
                            ...
                            className={`text-gray-300 hover:text-primary text-base font-medium ${pathname === link.href ? 'text-primary' : ''}`  }
                            >
                            {link.label}
                            </Link>
                        ))}
                        <Link href="/packages" className="w-full md:w-auto">
                            <button className="w-full mt-4 cursor-pointer bg-primary text-[#181611] font-bold py-3 rounded-lg" onClick={() => setOpen(!open)}>
                                Book Safari
                            </button>
                        </Link>
